...
| EventSourcingDB extensions (returned when reading) | `hash`, `predecessorhash`, `signature`, `traceparent`, `tracestate` | Integrity and tracing metadata, not payload. `hash` and `predecessorhash` chain the events of a subject cryptographically (SHA-256); `signature` is `null` unless the server has a signing key configured. |
...
- **`type`** must use reverse domain notation with at least two dot-separated segments (e.g., `io.example.book-acquired`).
...
A `predecessorhash` of all zeros marks the first event of a subject.
